...
Cache::Block* CacheSimulator::find_block(uint32_t address) const {
  ...
  const CacheConfig& conf = _cache->get_config();

  uint32_t tag = extract_tag(address, conf);
  uint32_t i = extract_index(address, conf);
...
Cache::Block* CacheSimulator::bring_block_into_cache(uint32_t address) const {
  ...
  uint32_t i = extract_index(address, config);
...
void CacheSimulator::write_access(uint32_t address, uint32_t word) const {
  ...
  const CacheConfig& conf = _cache->get_config();
  uint32_t offset = extract_block_offset(address, conf);
